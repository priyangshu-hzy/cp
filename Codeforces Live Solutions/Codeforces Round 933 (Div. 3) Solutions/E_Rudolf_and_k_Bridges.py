...
for _ in range(int(input())) :
    N, M, K, D = map(int,input().split()) ; Map = [list(map(int,input().split())) for _ in range(N)]

    A = [0] * N

    for i in range(N) :
        dp = [0] * M ; dp[0] = 1 ; h = [] ; heapq.heappush(h, (1, 0))

        for j in range(1, M) :
            while h[0][1] < j-D-1 : heapq.heappop(h)
            dp[j] = h[0][0]+Map[i][j]+1 ; heapq.heappush(h, (dp[j], j))
    ...
    for i in range(K, N) :
        cnt = cnt-A[i-K]+A[i] ; Ans = min(Ans, cnt) 
